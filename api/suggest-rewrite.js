...
//
// DESIGN STANCE (important, and worth stating in a pitch):
// This does NOT try to rewrite text so it "passes the check" — that would be
// compliance theatre, teaching people to launder non-compliant intent past a
// filter. Instead it suggests SOFTENING / REFRAMING toward genuinely more
// compliant language, and every suggestion is explicitly a draft for a human
// to approve, never an auto-applied edit.
//
// Two-tier approach:
//   1. Curated map — deterministic, safe, reviewed rewrites for common flagged
//      phrases. Fast, free, and predictable. Preferred when a match exists.
//   2. LLM fallback — for phrases not in the curated map, ask the model to
//      propose a compliant reframing. Clearly marked as AI-proposed + unreviewed.

const CURATED = {
  // solicitation / recommendation language -> neutral commentary framing
...
// See api/review.js for the rationale — Groq's free-tier TPM cap means a busy
// session can hit rate limits; retry with backoff before surfacing an error.
...
export default async function handler(req, res) {
  ...
  // Tier 1: curated map (exact or contained match)
...
Respond with ONLY valid JSON: {"suggestion":"...","rationale":"one short sentence"}`;
  ...
  try {
    const g = await fetchWithRetry("https://api.groq.com/openai/v1/chat/completions", {
      method: "POST",
